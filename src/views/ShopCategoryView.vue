<template>
  <div class="w-full h-full">
    <div v-if="this.category" class="flex w-full justify-center h-2/3 md:h-1/5 bg-black bg-opacity-25">
    ...
    <div class="flex justify-center">
      <div class="py-32 w-2/3">
        <div class="flex flex-col md:flex-row grid grid-cols-1 md:grid-cols-4 gap-4 items-center w-full">
          <ProductObjectTile v-for="product in products" :key="product.id" :object="product" :objectType="'product'" />
        </div>
      </div>
    </div>
  </div>
